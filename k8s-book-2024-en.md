...
This is where Kubernetes _Services_ come into play by providing reliable networking for
groups of Pods.
...
All the major cloud platforms offer a _hosted Kubernetes_ service. This is a model where
...
Serverless functions run in standard Pods but require apps likeKnative^4 to extend
...
_nodeSelectors_ are the simplest way of running Pods on specific nodes. You give the
...
Deploying a Pod is an _atomic operation_. This means a Pod only starts servicing requests
...
Deployments are the most popular way of running _stateless_ apps on Kubernetes. They
...
You’ll sometimes hear people refer to _multi-dimensional autoscaling._ This is jargon
...
with the fix. To do this, you update the **sameDeploymentYAMLfile** with the new
...
This will belocalhost:8080if you’re on a local Docker Desktop cluster. It’ll be a public
IP or DNS name if your cluster is in the cloud.
Figure 6.6 shows a browser accessing the app onlocalhost:8080.
```
```
Figure6.6
```
...
Before continuing, it’s vital you understand that all _update_ operations are actually
**_replacement_** operations. When you update a Pod, you’re actually deleting it and replacing
it with a new one. Pods are _immutable objects,_ so you never change or update them after
they’re deployed.
...
metadata:
labels:
app: hello-world <<==== Matches the label selector
<Snip>
...
previous version said **Kubernetesrocks!** , this one says **WebAssemblyiscoming!**
...
requests. Fortunately, Kubernetes has a solution — _Service objects_ sit in front of one or
more identical Pods and expose them via a **_reliable_** DNS name, IP address, and port.
...
```
Figure7.1-ClientsaccessingPodsviaaService
```
```
...
Looking back to Figure 7.1, the client sends traffic to the Service on eitherapp1:8080or
...
Services use _labels_ and _selectors_ to know which Pods to send traffic to. This is the same
...
It works like this.
You create a Service, and the _EndpointSlice controller_ automatically creates an associated
_EndpointSlice object_. Kubernetes then watches the cluster, looking for Pods matching
...
Older versions of Kubernetes used an _Endpoints_ object instead of EndpointSlices.
...
network fabric and is onlyaccessiblefrominsidethecluster. This means:
```
...
- They use high-numbered ports between30000-32767
- Clients need to know the names or IPs of nodes, as well as whether nodes are
    healthy

This is why most people use _LoadBalancer Services_ instead.

**LoadBalancer Services - Accessing apps via load balancers**

_LoadBalancer Services_ are the easiest way of exposing Services to external clients. They
...
7: Kubernetes Services 100

```
Figure7.4-LoadBalancerService
```
...
experience will be the same, but you’ll use local constructs such aslocalhost.
...
assigned an **EXTERNAL-IP** of212.2.245.220. If you’re on a local cluster such as Docker
Desktop, the **EXTERNAL-IP** might showlocalhost.
...
this. The NodePort is randomly assigned from between30000-32767.
...
**Session Affinity** allows you to control session _stickiness_ — whether or not connec-
tions from the same client always go to the same Pod. The default is _None_ and allows
connections from the same clients to be forwarded to any Pods. You should try the
_ClientIP_ option if your clients and Pods store state in Pods and require session stickiness.
However, this is an _anti-pattern_ as microservices apps should be designed for process
...
probably be exposed on your laptop’slocalhostinterface. Either way, you can connect
...
We’ll capitalize _Ingress_ as it’s a resource in the Kubernetes API. We’ll also be using the
terms _LoadBalancer_ and _load balancer_ as follows:

- _LoadBalancer_ refers to a Kubernetes Service object of **type=LoadBalancer**
- _load balancer_ refers to one of your cloud’s internet-facing load balancers
...
It does this by creating a single cloud load balancer on port 80 or 443 and using _host-
based_ and _path-based_ routing to map connections to different Services on the cluster.
...
2.A controller
```
The resource _defines_ the routing rules, and the controller _implements_ them.
...
Once you have an _Ingress controller_ , you deploy _Ingress resources_ with rules telling the
controller how to route requests.

On the topic of _routing,_ Ingress operates at layer 7 of the OSI model, also known as the
_application layer_. This means it can inspect HTTP headers and forward traffic based on
hostnames and paths.
...
```
Host-basedexample Path-basedexample BackendK8sService
...
the HTTP headers. In this example,shield.mcu.comis routed to the **shield** ClusterIP
Service, andhydra.mcu.comis routed to the **hydra** Service. The logic is the same for
path-based routing, and we’ll see both in the hands-on section.

```
Figure8.1Host-basedrouting
```
...
If you have a local cluster, such as Docker Desktop, you’ll work withlocalhostand
...
_controller_ Pod is running. It may take a few seconds to enter the running phase, and
...
4.Configure DNS name resolution to thatshield.mcu.com,hydra.mcu.com, and
mcu.compoint to the load balancer

Figure 8.2 shows the overall architecture using host-based routing.

```
Figure8.2Host-basedrouting
```
Traffic flow to the **shield** Pods will be as follows:

```
1.Client sends traffic toshield.mcu.comormcu.com/shield
...
example, traffic hitting the load balancer on themcu.com/shieldpath will have the path
rewritten tomcu.com/. You’ll see an example shortly. This annotation is specific to the
...
```
- Lines 10-19 define a host-based rule for traffic arriving onshield.mcu.com
- Lines 20-29 define a host-based rule for traffic arriving onhydra.mcu.com
- Lines 30-39 define a path-based rule for traffic arriving onmcu.com/shield
- Lines 40-49 define a host-based rule for traffic arriving onmcu.com/hydra

Let’s look at an example of a host-based rule and then a path-based rule.
The following _host-based_ rule triggers on traffic arriving viashield.mcu.comat the root
...
- host: shield.mcu.com <<==== Traffic arriving via this hostname
    [http:](http:)
       ...
       - path: / <<==== Arriving at root (no subpath specified)
          pathType: Prefix
          backend: <<==== The next five lines reference an
...
```
The following path-based rule triggers when traffic arrives onmcu.com/shield. It gets
...
name. If you’re on a local cluster, it’ll probably belocalhost. The **PORTS** field can be 80
or 443.
...
might belocalhoston local clusters.
...
The rules define the mappings between _hosts_ , _paths_ , and _backends_. Remember that
...
they arrived on root “/”. This is a _best-effort_ approach, and as you’ll see later, it doesn’t
work with all apps.
...
```
Figure8.3Cloudback-endloadbalancerconfiguration
```
...
The only thing left to configure is DNS name resolution so thatshield.mcu.com,
hydra.mcu.comandmcu.comall send traffic to the load balancer.
...
Create three new lines mappingshield.mcu.com,hydra.mcu.com, andmcu.comto the
IP of the load balancer. Use the IP from the output of a kubectl get ing mcu-all
command. If yours sayslocalhost, use the127.0.0.1IP address.
```
...
```
With this done, any traffic you send toshield.mcu.com,hydra.mcu.com, ormcu.com
...
the headers. Traffic forshield.mcu.comgoes to the svc-shield Service, and traffic for
hydra.mcu.comgoes to the svc-hydra Service.
```

8: Ingress 121

```
Figure8.4-host-basedrouting
```
Notice that requests tomcu.comare routed to the _default backend_. This is because you
didn’t create an Ingress rule formcu.com. Depending on your Ingress controller, the
...
message saying, _response 404 (backend NotFound), service rules for [ / ] non-existent_.
...
For path-based routing like this, the Ingress uses the _rewrite targets_ feature as specified in
...
```
Pods Services Ingresscontrollers Ingressresources
...
Finally, **don’tforgettorevertyour/etc/hostsfileifyouaddedmanualentries
earlier**.
...
correct way to write it is Wasm , notWASM. However, be kind to people and don’t
...
```
9: WebAssembly on Kubernetes 124
```
### Wasm Primer.
...
```
9: WebAssembly on Kubernetes 125
```
```
...
are _architecture-dependent_ , meaning they’re **not** portable. For example, every container is
...
still an overhead, and many organizations end up with _image sprawl_. As an oversimplified
...
WebAssembly solves this and delivers on the **_build once, run everywhere_** promise!
...
```
WebAssembly delivers on the promise of _build once, run anywhere._
...
```
9: WebAssembly on Kubernetes 126
```
In fact, Wasm _cold starts_ are so fast that they don’t feel like cold starts. For example,
...
```
9: WebAssembly on Kubernetes 127
```
```
Figure9.2
```
...
9: WebAssembly on Kubernetes 128
...
Figure9.4
```
The code to interface with containerd is always _runwasi_^9 , but each shim can embed a
...
runtime. In each shim, the embedded Wasm runtime creates the _Wasm host_ and executes
...
The way to do this is a combination of _node labels_ and _RuntimeClass_ objects. Node 2 in
...
Go tohttps://www.rust-lang.org/tools/installto install Rust.
...
scaffold a simple Spin app that responds to web requests on port 80 on the/tkbpath.
...
```
9: WebAssembly on Kubernetes 132
```
```
...
```
Edit the src/lib.rs file so that it returns the text TheKubernetesBooklovesWasm!.
...
```
9: WebAssembly on Kubernetes 133
```
```
$ spin build
...
COPY spin.toml.
```
The **FROM scratch** line tells Docker to base the new image on the empty _scratch_ image
...
9: WebAssembly on Kubernetes 134
...
Notice how small the image is. Similar _hello world_ Linux containers are usually several
megabytes in size.
...
```
9: WebAssembly on Kubernetes 135
```
```
...
9: WebAssembly on Kubernetes 136
...
named according to the _containerd shim naming convention_ , which prefixes the shim name
...
```
9: WebAssembly on Kubernetes 137
```
```
...
```
9: WebAssembly on Kubernetes 140
```
```
$ kubectl apply \
...
```
9: WebAssembly on Kubernetes 141
```
```
...
the directory and all files in it. **Besuretodeletethecorrectdirectory!**
...
choose the cotext from the **Kubernetescontext** option.
...
— instead of compiling to something like _Linux on ARM_ , you compile to _Wasm_.
...
9: WebAssembly on Kubernetes 142
...
that are implemented as _containerd shims_. To run a Wasm app on a Kubernetes cluster
...
Other ways to run Wasm apps on Kubernetes exist. One alternative is using _crun_ instead
of containerd. See the **_Kwasm_** project if you need to add Wasm support to an existing
cluster.
...
Finding things on busy platforms like Kubernetes is hard, _service discovery_ makes it easy.
...
10: Service discovery deep dive 144
...
```
Developers are responsible for step 1 — ensuring apps know the _names_ of the other apps
...
- **Step1:** The developer configures **app-a** to talk to **app-b**
- **Step2:app-a** asks Kubernetes for the IP address of **app-b**
- **Step3:** Kubernetes returns the IP address
- **Step4:app-a** sends requests to **app-b’s** IP address


10: Service discovery deep dive 145
...
Every Kubernetes cluster has a built-in _cluster DNS_ that it uses as its service registry.
...
_service registration_ and _service discovery_ are both automatic.


10: Service discovery deep dive 146

```
Figure10.3-ClusterDNSarchitecture
```
...
10: Service discovery deep dive 147
...
The registration process is automatic because the cluster DNS is a _Kubernetes-native
application_ that watches the API server for new Services. Every time it sees a new one, it
...
10: Service discovery deep dive 148

```
Figure10.4-Serviceregistrationflow
```
...
```
App Servicename ClusterIP
Enterprise ent 192.168.201.240
Cerritos cer 192.168.200.217
```

```
10: Service discovery deep dive 149
```
```
Figure10.5
```
...
An _unqualified name_ is a short name such as **ent**. Appending a search domain converts it
to a fully qualified domain name (FQDN) such asent.default.svc.cluster.local.
The following extract is from a container’s **/etc/resolv.conf** file configured to send
service discovery requests (DNS queries) to the cluster DNS at10.96.0.10. It also lists
...
```
10: Service discovery deep dive 150
```
```
...
```
The container’s default gateway sends the traffic to the _node_ it’s running on. The node
...
```
10: Service discovery deep dive 151
```
```
Figure10.6
```
...
the traffic to the IP. However, ClusterIPs are on the _service network_ and the container
...
Every Kubernetes object gets a name in the _cluster address space_ , and you can partition the
address space with Namespaces.
The cluster address space is a DNS domain that we usually call the _cluster domain_. On
most clusters, it’scluster.local, and object names have to be unique within it. For
example, you can only have one Service called **cer** in the default Namespace, and it will
be calledcer.default.svc.cluster.local.
Long names like this are called _fully qualified domain names (FQDN)_ , and the format is
...
10: Service discovery deep dive 152
...
Object names must be unique _within_ a Namespace but not _across_ Namespaces. As a quick
...
10: Service discovery deep dive 153
...
10: Service discovery deep dive 154
...
Let’s see how service discovery works _within_ a Namespace and _across_ Namespaces.
...
The _Hello from the DEV Namespace_ response proves the connection reached the instance
in the **dev** Namespace.

The container automatically appendeddev.svc.cluster.localto the name and sent
...
```
10: Service discovery deep dive 157
```
### Troubleshooting service discovery.
...
- **EndpointSliceobjects:** Names pre-fixed with **kube-dns**
...
10: Service discovery deep dive 158
...
go toexplore.ggcr.dev/to browse theregistry.k8s.io/e2e-test-imagesrepo for
newer versions.
...
The query should return an IP address and the namekubernetes.default.svc.cluster.local.
...
Errors such as _nslookup: can’t resolve kubernetes_ are indicators that DNS isn’t working. A
...
10: Service discovery deep dive 159
...
tunately, the Kubernetes _persistent volume subsystem_ lets you connect enterprise-grade
...
include _block, file,_ and _object_ storage from various external systems that can be in the
cloud or your on-premises datacenters.

Figure 11.1 shows the high-level architecture.


11: Kubernetes storage 161

```
Figure11.1
```
The _storage providers_ are on the left. As mentioned, these are the external systems
...
the external storage systems with Kubernetes. Modern plugins use the _Container
Storage Interface (CSI)_ , which is an industry-standard storage interface for container
orchestrators such as Kubernetes. If you’re a developer writing storage plugins, the CSI
abstracts the internal Kubernetes machinery and allows you to develop _out-of-tree_.
...
_PersistentVolume, PersistentVolumeClaim,_ and _StorageClass._ We’ll also use their shortnames,
PV, PVC, and SC.
...
systems. We usually call these _providers_ or _provisioners_.

Each _provider_ supplies its own _CSI plugin_ and has unique features and configuration
options.


11: Kubernetes storage 163
...
repository refers to plugins as _drivers_.
...
```
Externaltier KubernetesStorageClassname CSIplugin
SSD sc-fast csi.xyz
...
```
As the name suggests, _StorageClasses_ let you define different classes of storage that apps
...
2. **metadata.name** should be meaningful, as it’s how **you** and other objects refer to
    the class
3.The terms _provisioner_ , _plugin_ , and _driver_ are sometimes used interchangeably
...
**ReadWriteMany** lets multiple PVCs bind to a volume in read-write (R/W) mode. _File_ and
_object_ storage usually support this mode, whereas _block_ storage usually doesn’t.
...
StorageClasses. It deletes the PV **andassociatedexternalstorage** when the PVC is
...
example, one SC might provision _high-performance AWS SSD storage in the AWS Mumbai
Region_ , while another might provision _slow AWS storage from a different AWS region._ Once
...
The demos will only work on **RegionalGKEclusters** like the one we showed you how
...
11: Kubernetes storage 171

The following commands show the PVC was successfully created. However, it’s in
the _pending_ state, and no PV has been created. This is because the **premium-rwo**
...
- name: ubuntu-ctr
    ...
    - name: data <<==== called data (see above)
       mountPath: /data <<==== to this directory
...
Go to the **ComputeEngine>Disks** tab of your Google Cloud Console and verify the
...
$ kubectl get sc
RECLAIM ALLOWVOLUME
NAME PROVISIONER POLICY VOLUMEBINDINGMODE EXPANSION
...
apiVersion: v1
kind: PersistentVolumeClaim <<==== Create a new PVC
metadata:
name: pvc2 <<==== Call it pvc2
spec:
accessModes:

- ReadWriteOnce
storageClassName: sc-fast-repl <<==== Base it on this SC
resources:
    requests:
       storage: 20Gi <<==== Create a 20Gi vol
---
apiVersion: v1 <<==== This Pod claims it
kind: Pod
metadata:
name: volpod
spec:
volumes:
- name: data
persistentVolumeClaim: <<==== Create from PVC
claimName: pvc2 <<==== with this name
<Snip>


11: Kubernetes storage 174
...
PVs, associated external volumes, **anddata** after you delete PVCs.

Run the following command to delete the PV. Be sure to use the PV name from your
environment.


11: Kubernetes storage 175
...
Delete the external disk on the Google Cloud from within the **ComputeEngine>Disks**
tab of your Google Cloud Console. The disk will show as _not in use_ and have exactly the
same name as the PV you deleted on your cluster. **_Failure to delete it on the GKE backend
will incur unwanted charges!_**
...
image, forcing you to performall of the following forevery application:
...
You then create three **_configurations_** for **dev** , **test** , and **prod** that you’ll apply at run time.
Each one will configure the NGINX container with the policy settings and credentials
for the correct environment. Other teams and applications can **_reuse_** the same hardened
...
ConfigMaps are first-class objects in the _core_ API group. They’re also **v1**. This tells us a
few things:
...
Here’s another example, but the _value_ contains a complete configuration file this time.
...
before we do that, let’s quickly mention _Kubernetes-native_ applications.

**ConfigMaps and Kubernetes-native apps**

_Kubernetes-native applications_ know they’re running on Kubernetes and can talk to the
...
- The name of the _key_ matches the name of the input file ( **cmfile.txt** )
- The _value_ stores the contents of the file


12: ConfigMaps and Secrets 183
...
because the _value_ entry contains an entire configuration file.
...
```
Figure12.3-MappingConfigMapentriestostartupcommands
```
Start a new Pod from the **startuppod.yml** file. The Pod will start, print **Firstname
NigellastnamePoulton** to the container’s logs and then quit (succeed). It might take
...
**FirstnameNigellastnamePoulton**.


12: ConfigMaps and Secrets 189
...
container’s filesystem via a _ConfigMap volume_ , and you’ve tested making updates.
...
The quick answer to this question is **_no_**. But here’s the slightly longer answer...
...
people use tools such as HashiCorp’sVault^10 for a more complete and secure secrets
management solution.
```
...
controller ensures _observed state_ matches _desired state_.
...
is a _zero-based index ordinal_ , which is a fancy way of saying _number starting from zero_.
...
_running_ and _ready_ before the StatefulSet controller starts **tkb-sts-1**. The same applies
to subsequent Pods — **tkb-sts-1** needs to be _running_ and _ready_ before **tkb-sts-2** starts etc.
See Figure 13.1
...
possible, StatefulSets use a _headless Service_ to create reliable and predictable DNS names
...
StatefulSet YAML as the _governing Service_.
...
$ kubectl get sc
VOLUME ALLOWVOLUME
NAME PROVISIONER RECLAIMPOLICY BINDINGMODE EXPANSION
...
It’s helpful to visualize Service objects with a head and a tail. The _head_ is the stable
...
```
volumeClaimTemplatename PodName PVCName
...
custom address space when you build your cluster, but most use thecluster.local
...
dullahan.default.svc.cluster.localname.
...
The output shows that clients asking aboutdullahan.default.svc.cluster.local
...
SECTION maps requests fordullahan.default.svc.cluster.localto the three Pods,
...
Changing the value to **Parallel** will cause the StatefulSet to act more like a _Deployment_
...
Recovering from _potential_ node failures is a lot more complex and depends on your Ku-
...
Kubernetes is _API-centric_ and the API is served through the _API server_. In this chapter,
...
_authentication_ module determines whether the request originates from **grant-ward** or
an imposter. Assuming it is **grant-ward** , the _authorization_ module (RBAC) determines
whether **grant-ward** has permission to create **Deployments** in the **terran** Namespace.
If the request passes authentication and authorization, _admission controllers_ ensure
...
_authenticate_ yourself with a photo ID, usually your passport. Assuming you pass the
passport authentication, you then present a ticket _authorizing_ you to board the plane. If
...
_authN_ , pronounced _“auth en”_.

Credentials are at the heart of authentication, and **_all requests to the API server include
credentials_**. It’s the responsibility of the authentication layer to verify them. If verifica-
tion fails, the API server returns an HTTP 401 and denies the request. If it succeeds, the
request moves on to authorization.

The authentication layer in Kubernetes is pluggable, and popular modules include _client
certs, webhooks,_ and integration with external identity management systems such as _Active
Directory (AD)_ and cloud-based _Identity Access Management (IAM)._ In fact, Kubernetes
does **not** have its own built-in identity database. Instead, it forces you to use an external
system. This avoids creating _yet another identity management silo_.

Out-of-the-box, most Kubernetes clusters support _client certificates_ , but you’ll want to
...
14: API security and RBAC 216

**Checking your current authentication setup**

Your cluster details and user credentials are stored in a _kubeconfig_ file. Tools like **kubectl**
...
Here’s what a kubeconfig file looks like. As you can see, it defines a _cluster_ and a _user_ ,
combines them into a _context_ , and sets the default context for **kubectl** commands. The
...
Josh delete ServiceAccounts Josh can delete
ServiceAccounts
```
RBAC is enabled on most Kubernetes clusters and is a _least-privilege deny-by-default
system_. This means everything is locked down, and you need to create _allow rules_ to open
things up. In fact, Kubernetes doesn’t support _deny rules_ , it only supports _allow rules_.
...
_Roles_ define a set of permissions, and _RoleBindings_ bind them to users.
...
```
```
apiGroup resource KubernetesAPIpath
...
```
```
Kubernetesverb(s) HTTPmethod Commonresponses
...
Namespaces. On the other hand, _ClusterRoles_ and _ClusterRoleBindings_ are cluster-wide
...
```
Figure14.2-CombiningClusterRolesandRoleBindings
```
...
The PolicyRule section shows this Role has access to all _verbs_ on all _resources_ in all
Namespaces. This is the equivalent of _root_ and is a powerful and dangerous set of
permissions.
...
14: API security and RBAC 225

```
Figure14.3-Mappingkubectluserstoclusteradmin
```
...
The names tell you a lot. _Mutating_ controllers check for compliance and can modify
requests, whereas _validating_ controllers check for compliance but cannot modify
requests.


14: API security and RBAC 226
...
with terms like _RESTful_. If that’s you, this chapter will blow away the confusion and get
...
Finally, Pods, Services, StatefulSets, StorageClasses, and more are all _resources_ in the
API. However, it’s common to call them _objects_ when deployed to a cluster. We’ll use the
terms _resource_ and _object_ interchangeably.

### Kubernetes API big picture..

Kubernetes is _API-centric_ — all resources are defined in the _API,_ and all communication
goes through the _API server._
...
tools. The point is, no matter _how_ you generate requests, they always go to the API
...
What does it mean to persist an object to the cluster store in its _serialized state_?
...
end to the API and is a bit like _Grand Central station_ for Kubernetes — everything talks to
...
ensure the API server can respond to requests quickly. If you’re using _hosted Kubernetes_ ,
...
The API is _RESTful_. This is jargon for a modern web API that accepts CRUD-style
requests via standard HTTP methods. _CRUD-style operations_ are simple **_c_** _reate,_ **_r_** _ead,_
**_u_** _pdate,_ **_d_** _elete_ operations, and they map to the standard POST, GET, PUT, PATCH, and
DELETE _HTTP methods_.
...
term _verb_ to refer to CRUD operations.


15: The Kubernetes API 233

```
K8sCRUDverb HTTPmethod kubectlexample
...
You’ll hear the terms REST and RESTful a lot. REST is short for **RE** presentational **S** tate
**T** ransfer and is the de facto standard for communicating with web-based APIs. Systems
that use REST, such as Kubernetes, are often referred to as RESTful.

_REST requests_ comprise a _verb_ and a _path_ to a resource. Verbs relate to actions and map to
...
on yourlocalhostadapter and handle all authentication. Feel free to use a different
port.


```
15: The Kubernetes API 234
```
```
...
HTTP200(OK) response code, and Kubernetes will respond with a serialized JSON list
...
Lines starting with **>** are header data _sent_ by **curl**. Lines starting with **<** are header data
_returned_ by the API server.
...
sist objects — **C** reate, **R** ead, **U** pdate, **D** elete. As previously mentioned, the Kubernetes
...
```
Figure15.3-SimplifiedviewofKubernetesAPI
```
```
...
On the topic of REST paths, **GVR** stands for **g** roup, **v** ersion, and **r** esource, and can be
...
new resources to _named groups_.

**Named API groups**

The _named API groups_ are the future of the API, and all new resources get added to
named groups. Sometimes, we refer to them as _sub-groups_.
...
when the API was first designed — everything was _“just in the API”._
...
in as _alpha_ , progress through _beta_ , and eventually graduate as _Generally Available (GA)_. We
sometimes refer to GA as _stable_.

**Alpha** resources are experimental and should be considered _hairy and scary_. Expect
...
**Beta** resources are considered _pre-release_ and are starting to look like the final GA
...
The final phase after beta is _Generally Available (GA)_ , sometimes referred to as _stable_.
...
- Create your custom _resource_
- Write your custom _controller_
...
15: The Kubernetes API 250

Resources in the Kubernetes API are usually _objects_. However, they can also be _lists_ or
_operations_. The vast majority are objects, so we sometimes use the terms _resources_ and
_objects_ to mean the same thing. It’s common to refer to their API definitions as resources
or resource definitions, whereas running instances on a cluster are often referred to as
objects. For example, the Pod _resource_ exists in the core API group, and there are five Pod
_objects_ running in the default Namespace.
...
them in production. GA resources are considered _production-grade_ , and Kubernetes has
...
16: Threat modeling Kubernetes 252
...
only used and trusted _within_ the Kubernetes cluster. This requires careful approval of
...
```
16: Threat modeling Kubernetes 253
```
```
...
16: Threat modeling Kubernetes 254
...
- **Denialofservice** : Tampering with the resource to make it unusable
- **Elevationofprivilege** : Tampering with a resource to gain additional privileges
...
Generally speaking, tampering happens either _in transit_ or _at rest_. In transit refers to data
while it is being transmitted over the network, whereas at rest refers to data stored in
memory or on disk.

TLS is a great tool for protecting against _in-transit_ tampering as it provides built-in
integrity guarantees that warn you when data has been tampered with.

The following recommendations can also help prevent tampering with data when it is _at
rest_ in Kubernetes:
...
```
16: Threat modeling Kubernetes 255
```
...
16: Threat modeling Kubernetes 256
...
At a very high level, _repudiation_ creates doubt about something. _Non-repudiation_ provides
...
```
16: Threat modeling Kubernetes 257
```
```
{
"kind":"Event",
...
trust the logs, and their contents can be _repudiated_.
...
```
16: Threat modeling Kubernetes 258
```
```
...
16: Threat modeling Kubernetes 259
...
16: Threat modeling Kubernetes 260
...
Limiting _Kubernetes objects_ can also be a good practice. This includes limiting things such
...
16: Threat modeling Kubernetes 261
...
16: Threat modeling Kubernetes 262
...
16: Threat modeling Kubernetes 263

You should run multiple authorizers at the same time. For example, it’s common to use
the _RBAC_ and _node_ authorizers.

_RBAC mode_ lets you restrict API operations to sub-sets of users. These _users_ can be
...
requested operation. For example, can _Mia create Pods_? In this example, _Mia_ is the user,
_create_ is the operation, and _Pods_ is the resource. Authentication makes sure that it really
is Mia making the request, and authorization determines if she’s allowed to create Pods.

_Webhook mode_ lets you offload authorization to an external REST-based policy engine.
...
_Node authorization_ is all about authorizing API requests made by kubelets (Nodes). The
...
The _root_ user is the most powerful user on a Linux system and is always User ID 0 (UID
...
```
16: Threat modeling Kubernetes 264
```
```
...
```
16: Threat modeling Kubernetes 265
```
```
...
run. Enter _capabilities_.
Time for a quick bit of background.

We’ve already said the root user is the most powerful user on a Linux system. However,
its power is a combination of lots of small privileges that we call _capabilities_. For example,
...
local routing table and configuring local interfaces. The root user holds every _capability_
...
With this in mind, many container runtimes implement a set of _sensible defaults_ that
allow most processes to run without _leaving all the doors open_. While sensible defaults
...
```
16: Threat modeling Kubernetes 266
```
```
...
16: Threat modeling Kubernetes 267

2. **Blocking** : Blocks all syscalls. It’s extremely secure but prevents a Pod from doing
    anything useful.
3. **RuntimeDefault** : Forces a Pod to use the seccomp profile defined by its container
    runtime. This is a common place to start if you still need to create a custom profile.
    Profiles that ship with container runtimes are designed to be a balance of _usable_
    and _secure_. They’re also thoroughly tested.
...
Custom profiles operate the _least privilege_ model and are the preferred approach from a
...
process is called the _parent_ process, and the copy is called the _child_ process.

By default, Linux allows a _child_ process to claim more privileges than its _parent_. This
...
16: Threat modeling Kubernetes 268
...
- **PodSecurityStandards(PSS)** are policies that specify required Pod security
    settings
- **PodSecurityAdmission(PSA)** enforces one or more PSS policies when Pods are
    created
...
Every Kubernetes cluster gets the following three PSS _policies_ that are maintained and
...
**Baseline** implements sensible defaults. It’s more secure than the _privileged_ policy but
less secure than _restricted_.
...
Namespace level and is implemented as a _validating admission controller_.
...
16: Threat modeling Kubernetes 269
...
16: Threat modeling Kubernetes 270
...
16: Threat modeling Kubernetes 271
...
Namespace. Using this flag **willnot** apply the policy.
...
Go ahead and apply the policy to the Namespace and see if it impacts thepsa-podthat is
already running.


16: Threat modeling Kubernetes 272
...
The following example applies three labels to the **psa-test** Namespace. They _enforce_ the
**baseline** policy, and _warn_ and _audit_ against the **restricted** policy. This is a good way
...
16: Threat modeling Kubernetes 273
...
Many cloud providers implement _confidential computing_ services such as _confidential
virtual machines_ and _confidential containers_ that Kubernetes can leverage to secure _data in
use_ by enabling memory encryption for container workloads, etc. Some cloud providers
even offer it as part of their hosted Kubernetes services.

An up-to-date third-partysecurity audit of Kubernetes^11 was published in April
...
Finally, theCloud Native Security Whitepaper^12 is worth reading as a way to level up
...
```
17: Real-world Kubernetes security 275
```
```
...
```
17: Real-world Kubernetes security 276
```
```
Figure17.2-Usingapprovedbaseimages
```
...
```
17: Real-world Kubernetes security 277
```
...
within your organization. These will typically be from a _trusted_ source and vetted by
...
17: Real-world Kubernetes security 278
...
_vulnerability scanning_. These services scan your images at a binary level and check their
...
scans images and automatically fails anything using an image with known _critical_
vulnerabilities.

However, some scanning solutions are better than others and will allow you to create
highly customizable policies.

For example, a Python _method_ that performs TLS verification might be vulnerable to
...
implement _policy as code_ rules.
...
```
17: Real-world Kubernetes security 279
```
```
...
```
17: Real-world Kubernetes security 280
```
### Workload isolation..
...
```
Cutting straight to the chase, Kubernetesdoesnotsupportsecuremulti-tenant
...
```
17: Real-world Kubernetes security 281
```
```
Namespaces and hard multi-tenancy
```
We’ll define _hard multi-tenancy_ as hosting untrusted and potentially hostile workloads
on shared infrastructure. However, as we said before, this isn’t _currently_ possible with
Kubernetes.
...
```
17: Real-world Kubernetes security 282
```
```
...
gVisor and KataContainers , both of which provide stronger levels of workload
...
1. **VirtualMachines:** Every workload gets its own dedicated kernel. It provides
    excellent isolation but is comparatively slow and resource-intensive.
2. **Namespacedcontainers:** All containers share the host’s kernel. These are fast and
    lightweight but require extra effort to improve workload isolation.
3. **Runeverycontainerinitsownvirtualmachine:** Solutions like these attempt to
...
4. **Usedifferentruntimeclasses:** This allows you to run all workloads as containers,
    but you target the workloads requiring stronger isolation to an appropriate
    container runtime.
5. **Wasmcontainers:** Wasm containers package Wasm (WebAssembly) apps in OCI
...
17: Real-world Kubernetes security 283
...
In Kubernetes, Pods communicate over an internal network called the _pod network_.
However, Kubernetes doesn’t implement the _pod network_. Instead, it implements a plugin
...
Most Kubernetes environments implement the pod network as a simple flat _overlay
network_ that hides any network complexity between cluster nodes. For example, you
...
17: Real-world Kubernetes security 284

```
Figure17.5-Encapsulationonoverlaynetwork
```
**Kubernetes and BGP**
...
BGP routing is similar and happens through a network of _peers_ that help each other find
routes.
...
```
Figure17.6-NoencapsulationonBGPnetwork
```

```
17: Real-world Kubernetes security 285
```
```
...
- Allow traffic from the10.0.0.0/24network
- Disallow traffic from the192.168.0.0/24network
...
```
17: Real-world Kubernetes security 286
```
```
...
```
- _Node management_ activities that you cannot perform via the Kubernetes API
- _Break the Glass_ activities, such as when the API server is down
...
```
17: Real-world Kubernetes security 287
```
```
...
tool called kube-bench^13 to run the CIS tests against your cluster and generate reports.
...
```
17: Real-world Kubernetes security 288
```
...
17: Real-world Kubernetes security 289
...
- _Privileged Pod creation by a human user:_ Privileged Pods can often gain root-level
...
- _Exec sessions by human users:_ Exec sessions grant _shell-like_ access to containers and
...
1. _Crawl_ : Threat modeling your existing apps will help you understand their current
    security posture. For example, which of your existing apps do and don’t communi-
    cate over TLS.
2. _Walk_ : When moving to Kubernetes, ensure the security posture of these apps
    remains unchanged. For example, if an app doesn’t communicate over TLS, do **not**
    change this as part of the migration.
3. _Run_ : Start improving the security of applications after the migration. Start with
...
```
17: Real-world Kubernetes security 290
```
### Real-world example.
...
```
Term Definition(accordingtoNigel)
Admission controller Code that validates or mutates resources to enforce
policies. Runs as part of the API admission chain
immediately after authentication and authorization.
```
```
Annotation Object metadata that can be used to expose alpha or beta
capabilities or integrate with third-party systems.
```
```
API Application Programming Interface. In the case of
...
resources are usually located in thenetworking.k8s.io
API group.
```
```
API resource All Kubernetes objects, such as Pods, Deployments, and
Services, are defined in the API as resources.
```
```
API Server Exposes the API on a secure port over HTTPS. Runs on
the control plane.
```
```
Cloud controller manager Control plane service that integrates with the
...
```
```
Cloud native A loaded term that means different things to different
...
```
Term Definition(accordingtoNigel)
Cluster A set of worker and control plane nodes that work
together to run user applications.
```
```
Cluster store Control plane feature that holds the state of the cluster
and apps. Typically, it is based on theetcddistributed
data store and runs on the control plane. It can be
deployed to its own cluster for higher performance and
higher availability.
```
```
ConfigMap Kubernetes object used to hold non-sensitive
...
```
```
Container Lightweight environment for running modern apps.
...
```
```
Container Network Interface (CNI) Pluggable interface enabling different network
...
```
```
Container runtime Low-level software running on every cluster Node
...
```
```
Container Runtime Interface (CRI) Low-level Kubernetes feature that allows container
...
```
```
Container Storage Interface (CSI) Interface enabling external 3rd-party storage systems to
...
```
```
containerd Industry-standard container runtime used in most
...
```
```
Control plane The brains of every Kubernetes cluster. Comprises the
API, API server, scheduler, all controllers, and more.
These components run on all control plane nodes of every
cluster.
```
```
control plane node A cluster node hosting control plane services. Usually, it
doesn’t run user applications. You should deploy 3 or 5
for high availability.
```
```
cri-o Container runtime. Commonly used in
OpenShift-based Kubernetes clusters.
```
```
CRUD The four basic Create, Read, Update, and Delete
operations used by many storage systems.
```
```
Custom Resource Definition (CRD) API resource used for adding your own resources to the
Kubernetes API.
```
```
Data plane The worker Nodes of a cluster that host user
applications.
```
```
Deployment Controller that deploys and manages a set of stateless
Pods. Performs rollouts and rollbacks, and can self-heal.
Uses a ReplicaSet controller to perform scaling and
self-healing operations.
```
```
Desired state What the cluster and apps should be like. For example,
...
```
```
Endpoints object Up-to-date list of healthy Pods matching a Service’s
label selector. Basically, it’s the list of Pods a Service will
send traffic to. Might eventually be replaced by
EndpointSlices.
```
```
etcd The open-source distributed database used as the cluster
store on most Kubernetes clusters.
Ingress API resource that exposes multiple internal Services
...
```
```
Ingress class API resource that allows you to specify multiple
different Ingress controllers on your cluster.
```
```
Init container A specialized container that runs and completes before
the main app container starts. Commonly used to
check/initialize the environment for the main app
container.
JSON JavaScript Object Notation. The preferred format for
sending and storing data used by Kubernetes.
```
```
K8s Shorthand way to write Kubernetes. The “8” replaces the
...
```
```
kubectl Kubernetes command line tool. Sends commands to the
API server and queries state via the API server.
```
```
Kubelet The main Kubernetes agent running on every cluster
Node. It watches the API Server for new work
assignments and maintains a reporting channel back.
```
```
Kube-proxy Runs on every cluster node and implements low-level
...
```
```
Label Metadata applied to objects for grouping. Works with
...
```
```
Label selector Used to identify Pods to perform actions on. For
...
```
Term Definition(accordingtoNigel)
Microservices A design pattern for modern applications. Application
...
```
```
Namespace A way to partition a single Kubernetes cluster into
...
```
```
RBAC Role-based access control. Authorization module that
...
```
Term Definition(accordingtoNigel)
Reconciliation loop A controller process watching the state of the cluster via
...
```
```
ReplicaSet Runs as a controller and performs self-healing and
scaling. Used by Deployments.
```
```
REST REpresentational State Trasfer. The most common
...
```
```
Secret Like a ConfigMap for sensitive configuration data. A
way to store sensitive data outside of a container image
and have it inserted into a container at runtime.
```
```
Service Capital “S”. Kubernetes object for providing network
...
```
```
Service mesh Infrastructure software that enables features such as
...
```
```
Sidecar A special container that runs alongside and augments a
...
```
```
StatefulSet Controller that deploys and manages stateful Pods.
Similar to a Deployment, but for stateful applications.
```
```
Storage Class (SC) Way to create different storage tiers/classes on a cluster.
...
```
Term Definition(accordingtoNigel)
WebAssembly (Wasm) Secure sandboxed virtual machine format for executing
apps.
```
```
Worker node A cluster node for running user applications. Sometimes
called a “Node” or “worker”.
```
```
YAML Yet Another Markup Language. The configuration
...
icons in the style of _digital rain_ code from the Matrix movies. There’s also a hidden
...
Feel free to email me attkb@nigelpoulton.comif you want to suggest content or fixes.
